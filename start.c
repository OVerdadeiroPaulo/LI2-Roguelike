...
void Build_rios(STATE *s,int i, int j, int size_rio ,int size_rio_max ,int altura_rio ,int altura_rio_max , int size_rio_1,
                    int size_rio_max_1, int altura_rio_1, int altura_rio_max_1,int center_rio_x ,int center_rio_y ,int radius_rio){

        // geracao de rios 
        // clausula para colocar paredes ao redor da gruta, para nao criar uma gruta infinita.
    if (j > size_rio && j < size_rio + size_rio_max  && i > altura_rio && i < altura_rio + altura_rio_max) { 
        ...
        // clausula para colocar paredes ao redor da gruta, para nao criar uma gruta infinita.
    if (j > size_rio_1 && j < size_rio_1 + size_rio_max_1  && i > altura_rio_1 && i < altura_rio_1 + altura_rio_max_1) {
        if (s->mapaMid [i][j].is_wall != TRUE){
            s->mapaMid[i][j].is_grass = FALSE;
            s->mapaMid[i][j].is_water = TRUE;
        }
...
void Build_dungeon_Mid(STATE *s) {
    ...
    int n_grass;
    int dungeon_dataMID_grass [HEIGHT_dungeon][WIDTH_dungeon];
    ...
    for (int i = 0; i < HEIGHT_dungeon; i++) {
        for (int j = 0; j < WIDTH_dungeon; j++) {
            ...
            if (j > 0 && j < WIDTH_dungeon-1 && i > 0 && i < HEIGHT_dungeon-1){    // clausula para garantir que a funcao nao procura paredes fora do array.
               ...
               if (s->mapaMid [i - 1][j - 1].is_grass == TRUE) n_grass++;
               if (s->mapaMid[i - 1][j].is_grass == TRUE) n_grass++;
               if (s->mapaMid[i - 1][j + 1].is_grass == TRUE) n_grass++;
               if (s->mapaMid[i][j - 1].is_grass == TRUE) n_grass++;
               if (s->mapaMid[i][j + 1].is_grass == TRUE) n_grass++;
               if (s->mapaMid[i + 1][j - 1].is_grass == TRUE) n_grass++;
               if (s->mapaMid[i + 1][j].is_grass == TRUE) n_grass++;
               if (s->mapaMid[i + 1][j + 1].is_grass == TRUE) n_grass++;
            ...
            // Clausula para escolher o conteudo de cada celula.
            if (s->mapaMid[i][j].is_water == TRUE) {
                    // numeros de probabilidades maiores para garantir construcao do deserto.
                if (n_grass >= 3)
                    dungeon_dataMID_grass [i][j] = TRUE;
                else
                    dungeon_dataMID_grass [i][j] = FALSE;
            }
            else {
                if (n_grass >= 3)
                    dungeon_dataMID_grass [i][j] = TRUE;
    ...
    for (int i = 0; i < HEIGHT_dungeon; i++) {
         for (int j = 0; j < WIDTH_dungeon; j++) {
                // clausula para colocar paredes ao redor da gruta, para nao criar uma gruta infinita.
            if (j == 0 || j == WIDTH_dungeon-1 || i == 0 || i == HEIGHT_dungeon-1)
                s->mapaMid [i][j].is_wall = TRUE;
            else {
                s->mapaMid [i][j].is_wall = dungeon_dataMID [i][j];
                s->mapaMid [i][j].is_grass = dungeon_dataMID_grass [i][j];   
                Build_rios(s, i, j, size_rio, size_rio_max, altura_rio,altura_rio_max, size_rio_1,size_rio_max_1,altura_rio_1,altura_rio_max_1,center_rio_x,center_rio_y,radius_rio);
            }
        }
    }
