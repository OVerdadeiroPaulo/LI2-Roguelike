...
void Build_dungeon_Mid(STATE *s) {
    ...
    for (int i = 0; i < HEIGHT_dungeon; i++) {
        for (int j = 0; j < WIDTH_dungeon; j++) {
            ...
            if (j > 0 && j < WIDTH_dungeon-1 && i > 0 && i < HEIGHT_dungeon-1){   // clausula feita para nao procurar paredes fora do array e produzir o erro out of bounds.
...
void Initialize_rooms(STATE *s){
     for(int y = 0; y < HEIGHT_room; y++){
        for(int x = 0; x < WIDTH_room; x++){
            ...
            else if (y < HEIGHT_room-1 && y > 0 && x < WIDTH_room - 1 && x > 0 ){
                  s -> mapaEasy [y] [x].is_wall = TRUE;
                  }
        }
     }
...
int is_valid_room(STATE *s, int center_x, int center_y, int size_x, int size_y) {
    for (int y = center_y; y <= center_y + size_y; y++) {
        for (int x = center_x; x <= center_x + size_x; x++) {
            if ((s->mapaEasy[y][x].is_wall != TRUE || s->mapaEasy[y + 1][x].is_wall != TRUE || s->mapaEasy[y - 1][x].is_wall != TRUE || s->mapaEasy[y][x + 1].is_wall != TRUE || s->mapaEasy[y][x - 1].is_wall != TRUE) || (y <= 0 || y >= 55 || x <= 0 || x >= 250)) {
